...
def load_dataset(name, rm_pfeat=False, classes=[0, 1], to_numeric=True):
    ...
    if name == 'census':
        dataset = pd.read_csv(DATASET_PATH + 'census.csv', na_values='?', skipinitialspace=True)
        ...
        dataset = pd.read_csv(DATASET_PATH + 'default.csv', na_values='?', skipinitialspace=True)
        ...
        dataset = pd.read_csv(DATASET_PATH + 'compas.csv', na_values='', skipinitialspace=True)
...
def load_bank_dataset():
  bank_pd = pd.read_csv(DATASET_PATH + 'datasets/bank.csv')
